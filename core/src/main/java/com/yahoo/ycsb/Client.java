...
public class Client {
	...
	/**
	 * Defines the class to use as a ClientThread depending on the expected behavior.
	 * Extremely useful to define associate multiple behaviors with a Workload.
	 */
	public static final String LOAD_CLASS_PROPERTY = "loadclass";

	public static final String LOAD_CLASS_PROPERTY_DEFAULT = "com.yahoo.ycsb.behaviors.DefaultLoadClientThread";

	public static final String BENCHMARK_CLASS_PROPERTY = "benchmarkclass";

	public static final String BENCHMARK_CLASS_PROPERTY_DEFAULT = "com.yahoo.ycsb.behaviors.DefaultBenchmarkClientThread";
	...
	public static void main(String[] args) {
		...
		if (dotransactions) {
			...
			try {
				clientthreadclass = classLoader.loadClass(props.getProperty(BENCHMARK_CLASS_PROPERTY, BENCHMARK_CLASS_PROPERTY_DEFAULT));
			...
			try {
				clientthreadclass = classLoader.loadClass(props.getProperty(LOAD_CLASS_PROPERTY, LOAD_CLASS_PROPERTY_DEFAULT));
			} catch (ClassNotFoundException e) {
				e.printStackTrace();
				e.printStackTrace(System.out);
				System.exit(0);
		...
		for (int threadid = 0; threadid < threadcount; threadid++) {
			DB db = null;
			try {
				db = DBFactory.newDB(dbname, props, workload);
				Thread t = (ClientThread) clientthreadclass.getConstructor(ClientThread.getConstructorTypes()).newInstance(db, workload, threadid, threadcount, props, opcount / threadcount, targetperthreadperms);
				threads.add(t);
