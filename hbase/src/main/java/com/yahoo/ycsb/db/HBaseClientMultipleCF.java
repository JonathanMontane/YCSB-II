 ...
 */
public class HBaseClientMultipleCF extends com.yahoo.ycsb.DB {
	...
	private MultipleCF cfs = null;
	private ArrayList<byte[]> columnfamilies = new ArrayList<byte[]>();
	...
	public HashMap<String, Byte> _columnFamiliesMapper = new HashMap<String, Byte>();
	...
	public void init() throws DBException {
		...
		try{
			cfs = (MultipleCF) getWorkload();
			columnfamilies = cfs.getColumnFamilies();
		} catch (ClassCastException e){
			System.err.println("Error, the workload used is not compatible with multiple column families");
			throw new DBException("Invalid Worload Class");
		}
		...
		try {
			...
			if (fields == null) {
				...
				for (String field : fields) {
					byte[] _cf = cfs.getColumnFamily(table, field);
					g.addColumn(_cf, Bytes.toBytes(field));
				}
			}
			r = _hTable.get(g);
		} catch (IOException e) {
	...
	public int scan(String table, String startkey, int recordcount,
		...
		if (fields == null) {
			...
			for (String field : fields) {
				byte[] _cf = cfs.getColumnFamily(table, field);
				s.addColumn(_cf, Bytes.toBytes(field));
			}
		}
	...
	public int update(String table, String key,
		...
		for (Map.Entry<String, ByteIterator> entry : values.entrySet()) {
			...
			byte[] _cf = cfs.getColumnFamily(table, entry.getKey());
			p.add(_cf, Bytes.toBytes(entry.getKey()), entry
			      .getValue().toArray());
		}
